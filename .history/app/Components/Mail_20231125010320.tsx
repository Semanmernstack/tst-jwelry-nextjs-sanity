    ...
    <div className='flex flex-col items-center w-full  '>
        <form className="flex  flex-col p-3 " onSubmit={handleSubmit}>
            <div className='gap-2 flex w-full flex-col md:flex-row items-center'>
                <input type="email"
                 ...
                 />
                 <button className=" flex cursor-pointer h-9   text-white bg-zinc-950 p-2 text-xs sm:text-lg transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-95  duration-300  rounded-xl md:rounded-2xl " type='submit' disabled={!input}    >
                    Subscribe
                 </button>
            </div>
        ...
        {(mailOk || mailError ) && ( 
            <div className="mt-5 text-sm text-center justify-center font-extrabold">
             {mailOk ?(<p className=" text-xs text-center justify-center bg-red-900 p-1"> Subscription Sucessful !! </p>):(<p className="bg-green-500 p-1">You already Subscribed</p>)}
            </div>
        )}    
